...
test.describe("TanStack example dashboard flows", () => {
  ...
  test("covers organizations, billing tabs, checkout initialization, and callback states", async ({
    ...
    await page.route("**/api/auth/flutterwave/initialize-transaction", async (route) => {
    ...
    await expect
      ...
      .toMatchObject({
        callbackURL: "http://localhost:3000/billing/flutterwave/callback",
        quantity: 5,
      });
    ...
    await expect(page.getByText("Flutterwave->DB Synced Products")).toBeVisible();

    let productInitializePayload: unknown;
    await page.unroute("**/api/auth/flutterwave/initialize-transaction");
    await page.route("**/api/auth/flutterwave/initialize-transaction", async (route) => {
    ...
    await page.goto("/billing/flutterwave/callback");
